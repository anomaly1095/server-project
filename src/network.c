...
errcode_t net_send_ping(thread_arg_t *thread_arg, size_t thread_index, size_t client_index, const void *packet, const size_t packet_len)
{
  ...
  if (db_co_sel_all_by_fd(thread_arg->db_connect, &co, &nrow, thread_arg->total_cli_fds[thread_index][client_index].fd))
  {
    if (!co) 
      free(co);
      // check log in dblog file
      return __FAILURE__; 
  }
  // encrypt the ping msg  /// add this function in secrity.h
  
  // free the retreived co object 
  free(co);
  // send the ping 
  if (sendall(thread_arg, thread_index, client_index, packet, packet_len))
    return LOG(NET_LOG_PATH, E_SEND_PING, E_SEND_PING_M);
  // update connection status to next step 
 ...
 * @return Error code indicating success or failure.
 */
errcode_t net_recv_ping(thread_arg_t *thread_arg, size_t thread_index, size_t client_index, void *packet)
{
